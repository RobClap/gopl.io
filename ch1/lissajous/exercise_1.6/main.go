...
// Produce images in multiple colors by adding more values to palette and then
// displaying them by changing the third argument of SetColorIndex in some
// intresting way
package main
...
func lissajous(out io.Writer) {
	...
	for i := 0; i < nframes; i++ {
		...
		for t := 0.0; t < cycles*2*math.Pi; t += res {
			...
			img.SetColorIndex(size+int(x*size+0.5), size+int(y*size+0.5), uint8(index/12.0+1))
			index++
		}
		phase += 0.1
